...
### Real-device E2E checklist (human-only — this environment has no device, simulator, or browser-automation tool)

1. **Build**: `eas login` (once), then from the repo root: `eas build --profile preview --platform ios` and `eas build --profile preview --platform android`. The `preview` EAS profile points `EXPO_PUBLIC_API_URL` at the **preview** Worker (`slipstream-api-preview...`), not production — deliberate, so this pass can't leave test data in the real production D1. These are real, billed cloud builds (Expo account required) — nothing here can be triggered from this session.
2. **Install**: EAS prints a QR code / install link per platform for `internal` distribution — scan/open it on each physical device to install directly (no store submission needed for this pass).
