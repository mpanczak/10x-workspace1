...
**Phases 0, 1, 2, 3, 4, 5, 6, and 7 are done. Phase 8 is in progress** — NFR-002 measured empirically (p95 0.458s against 5,000 seeded rows), privacy guardrail re-confirmed, production promotion gate already proven in Phase 7. Only the real-device (iOS + Android) end-to-end pass remains, and it's human-only — no device, simulator, or EAS-authenticated account exists in this session.
...
## Phase 7 — CI/CD via GitHub Actions *(done)*
...
- [x] Verified locally that the CI typecheck step (`npx tsc --noEmit` from `backend/`) passes cleanly — **note**: the first real CI run still failed here, because the local check had silently been borrowing `typescript` from the repo root rather than `backend/`'s own tree (not a true CI-shaped dry-run). Fixed by adding `typescript` as an explicit `backend/` devDependency; see Edge Cases below.
...
- [x] Dry-run `workflow_dispatch` on `backend-deploy.yml` — completed by the human operator via the Actions tab: the reviewer gate correctly paused the job pending approval, then ran green end-to-end (migrate + `wrangler deploy --minify`) against production. Two non-fatal warnings surfaced in that real run and were fixed afterward (see Edge Cases): a `Cannot find base config file "expo/tsconfig.base"` warning, and a "multiple environments defined, no target specified" warning.

**Edge cases**
- **First real `backend-preview.yml` run failed at the typecheck step**: `npx tsc --noEmit` in CI resolved to an unrelated, deprecated npm package literally named `tsc` (`tsc@2.0.4`, not the TypeScript compiler) instead of `typescript`'s `tsc` binary, printing `This is not the tsc command you are looking for` and exiting 1. Root cause: `typescript` was never declared as a `backend/package.json` devDependency — every prior local `npx tsc --noEmit` (including this plan's own "verified locally" step) silently walked up to the **repo root's** `node_modules/typescript` (the Expo app's own devDependency) and used that, since `npx`'s local-binary lookup searches parent directories the same way Node's module resolution does. CI's `working-directory: backend` + `npm ci` only installs `backend/`'s own tree, so there was no local `tsc` to find and `npx` fell back to fetching the registry package named `tsc` instead. Fixed by adding `"typescript": "~5.9.2"` to `backend/package.json` devDependencies (matching the root project's pinned version) and re-verifying `npx tsc --noEmit` from a clean `backend/node_modules/.bin/tsc`. **Lesson**: a local-only verification step that "passes" from a subdirectory of a larger repo can be silently borrowing a dependency from an ancestor directory — worth re-verifying against a CI-shaped install (subdirectory-only, no root install) before trusting a "verified locally" claim as sufficient.
- **Production dry-run surfaced two non-fatal Wrangler warnings, both fixed afterward**:
  1. `Cannot find base config file "expo/tsconfig.base"` pointing at `../tsconfig.json` (the repo root's tsconfig, which extends Expo's base config). Root-caused by temporarily hiding the root `node_modules/expo` locally and reproducing the exact same warning via `wrangler deploy --minify --dry-run` from `backend/` — confirmed Wrangler's bundler was resolving a tsconfig *above* `backend/` (rather than only `backend/tsconfig.json`) when no explicit `tsconfig` path is set in `wrangler.jsonc`. Locally this stayed silent only because the repo-root `expo` package happens to be installed there too (Expo app's own devDependency) — CI's `backend`-only `npm ci` has no such fallback. Fixed by adding an explicit `"tsconfig": "tsconfig.json"` field to `backend/wrangler.jsonc`, confirmed via the same hidden-`expo` repro that the warning disappears once set. **Same failure shape as the `typescript` devDependency bug above**: a local check silently borrowing something from the repo root that isn't there in a clean `backend`-only install — worth treating "warning-free locally" with suspicion for anything under `backend/` given this pattern has now bitten twice.
  2. `Multiple environments are defined ... no target environment was specified for the deploy command` — `wrangler deploy --minify` in `backend-deploy.yml` didn't pass `--env`, leaving it to Wrangler's default (top-level/production) with a warning rather than an explicit statement of intent. Fixed by changing the deploy step to `wrangler deploy --minify --env=""`, matching Wrangler's own suggested fix for targeting the top-level environment unambiguously. Confirmed via local `--dry-run` that this produces the identical binding table (production `DB`/`BETTER_AUTH_URL`) with no warning.
...
**Done when**: Cloudflare CI token + both GitHub secrets + the `production` Environment (with required reviewers) exist; `backend-preview.yml` has run at least once from a real `backend/**` push with green steps; `backend-deploy.yml` has been dry-run via `workflow_dispatch` at least once and observably paused on the reviewer gate before completing. **Status: done, 2026-07-15.** Both workflows have run for real: `backend-preview.yml` green on `origin/main` after fixing the missing `typescript` devDependency; `backend-deploy.yml` dry-run via `workflow_dispatch` correctly paused on the reviewer gate, then completed a real production deploy. Two non-fatal warnings surfaced during that production run (`expo/tsconfig.base` resolution, ambiguous deploy environment) and were root-caused + fixed afterward — not yet re-verified against a real CI run (only local `--dry-run` so far); worth confirming clean on the next real `backend/**` push.

---

## Phase 8 — Verification / Smoke-Test Phase *(in progress)*

**Goal**: Close out every verification item deferred by earlier phases — NFR-002 measured empirically (not just estimated against a near-empty table), the privacy guardrail re-confirmed, the production promotion gate proven, and both PRD success-criteria paths (US-01/US-02) run end-to-end on real hardware, since simulator-only testing can't surface token-refresh/background-state auth bugs.

- [x] **NFR-002 — seeded-load p95 measurement**, done against **preview** D1 (never production): 5,000 synthetic rows seeded directly into `rides` (safe because `organizer_id` has no live FK constraint — see Phase 3's own note on that column), spread across the 6 real regions × 4 riding styles used elsewhere in this plan, with `start_at` randomized over the next ~180 days. Data volume choice: the PRD leaves this unresolved (`OQ-004: target_scale.data_volume`), so 5,000 rows was picked as a defensible order-of-magnitude for meaningful early growth, not a number derived from a real target. One throwaway test user authenticated, then 40 `GET /api/rides` requests sampled across varied filter/pagination combinations (unfiltered, single-field, combined filters, deep offset). Result: **p95 = 0.458s** (min 0.325s, max 0.604s) — comfortably under the 2s threshold. All 5,000 seeded rows and the test user (+ its `sessions`/`accounts` rows) deleted afterward, confirmed via `SELECT count(*)` returning 0.
  - **Caveat, stated plainly**: this measured backend/query latency from this environment's own network to Cloudflare's edge — it is not a real "standard mobile connection" measurement (higher/variable RTT, cellular jitter) as NFR-002's wording literally describes. The 4x latency headroom (0.46s vs. 2s budget) makes it very unlikely mobile network overhead alone would blow the budget, but a real on-device measurement (part of the native E2E pass below) is the stronger check.
- [x] **Privacy guardrail re-confirmed statically**: read `backend/src/db/schema.ts` end to end — the only location-shaped field anywhere in the schema is `rides.startAddress`, the one field FR-010 explicitly authorizes exposing to participants. No home address, no precise lat/lng, on `users` or `riderProfiles` or anywhere else. The guardrail holds by construction (the field doesn't exist to leak), not merely by an access-control check that could regress.
- [x] **Production promotion gate dry-run** — already satisfied in Phase 7 (see above): `workflow_dispatch` on `backend-deploy.yml` correctly paused on the required-reviewer gate, then completed a real production deploy once approved. Cross-referenced here since it's also this phase's own done-when condition.
- [ ] (MANUAL — human, needs a physical iOS device + a physical Android device + an authenticated Expo account) **Real native E2E** — both PRD success-criteria paths, on-device: see the step-by-step checklist below.

1. **Build**: `eas login` (once), then from the repo root: `eas build --profile preview --platform ios` and `eas build --profile preview --platform android`. The `preview` EAS profile points `EXPO_PUBLIC_API_URL` at the **preview** Worker (`slipstream-api-preview...`), not production — deliberate, so this pass can't leave test data in the real production D1. These are real, billed cloud builds (Expo account required).
   - **[x] Android — done, 2026-07-15.** Logged in as `slipstream-10x` (`jd7552820@gmail.com`, same account as Cloudflare). This project had never been linked to an EAS project — `eas init --id <id>` set that up, but couldn't auto-write `extra.eas.projectId` into `app.config.ts` since it's a dynamic (TS) config; added by hand. Build ran via `eas build --profile preview --platform android --non-interactive`: no local `keytool`, so EAS generated the Android keystore in the cloud (fine for this project — no separate local keystore requirement exists yet). `versionCode` initialized to `1` (first build, no remote version history). Finished clean — install link: `https://expo.dev/accounts/slipstream-10xs-team/projects/slipstream/builds/794e9323-e524-48ff-91bf-dd9c15982338`.
   - **[ ] iOS — not started.** Needs a paid Apple Developer Program account for code-signing/device registration, deliberately deferred (see the platform-choice decision above).
2. **Install**: open the link above (or scan its QR code) on the physical Android device to install directly via `internal` distribution — no store submission needed for this pass.
3. **Path 1 — participant** (PRD US-01): sign up (email+password) → complete rider profile (bio, riding style, experience level) → browse the ride list → filter by region + riding style → open a ride's detail (route description, organizer's motorcycle + experience level, start address) → join with one tap → confirm the app shows you on the participant list → send a message to the organizer.
4. **Path 2 — organizer** (PRD US-02): sign up → complete rider profile → add a motorcycle to the garage → create a ride (route description, motorcycle, riding style, purpose, start/arrival time, start address) → confirm it appears on the public list → confirm the Path-1 participant (run both roles, e.g. two devices or two accounts) shows up on your ride's participant list and their message arrives.
5. **Token-refresh / background-state check** (the specific reason this needs real hardware, not a simulator): after signing in, background the app (home button / app-switch) for a few minutes, then foreground it — confirm the session is still valid (no forced re-login) and a ride-list refresh still works. Repeat once more after backgrounding for longer (e.g. 15+ minutes) to probe token-refresh timing specifically.
6. **Google OAuth click-through** (deferred from Phase 3 for exactly this reason — needs a real device + real Google consent screen): sign in with Google on both platforms, confirm the redirect lands back in the app and a session is established. Use one of the Testing-status test users registered in Phase 0's Google Cloud OAuth consent screen.
7. **Cleanup**: after the pass, delete any test users/rides created during this checklist from **preview** D1 (`wrangler d1 execute slipstream-db-preview --remote --env preview`), matching every other phase's "leave no test data behind" discipline.

**Edge cases**
- **EAS builds are real, billed cloud builds** — not something to run speculatively; each `eas build` invocation consumes real build minutes/credits on the Expo account.
- **This environment cannot do any part of the real-device checklist** — no attached device, no simulator, no browser-automation tool, and `eas build`/`eas login` need an interactively-authenticated Expo account. Everything in that checklist is genuinely human-only, not a case of "the agent didn't try."
- **NFR-002's measurement is backend-only by necessity from this session** — the on-device pass (step 5 above) is the only way to fold in real mobile-network characteristics; treat the 0.458s figure as strong evidence the backend won't be the bottleneck, not as full NFR-002 sign-off on its own.
- **D1 statement-size limit hit while seeding**: an initial attempt at 400-row `INSERT` batches failed with `SQLITE_TOOBIG` (`statement too long`); dropping to 100-row batches (50 statements for 5,000 rows) succeeded. Worth remembering for any future D1 bulk-seed script — D1's per-statement size ceiling is tighter than a single `INSERT ... VALUES` with hundreds of rows.

**Done when**: NFR-002 measured against a seeded (not near-empty) dataset and confirmed under 2s p95 — done. Privacy guardrail re-confirmed — done. Production promotion gate dry-run — done (Phase 7). Both PRD paths (US-01, US-02) verified end-to-end via real EAS builds on physical iOS and Android hardware, including the token-refresh/background-state check and a real Google OAuth click-through — **open, human-only, not yet done**. **Status: in progress, 2026-07-15** — every automatable piece of this phase is done; only the real-device pass remains, and it cannot be completed from this session.
...
- `.github/workflows/backend-preview.yml`, `.github/workflows/backend-deploy.yml` — created in Phase 7 (done); `backend-deploy.yml`'s deploy step later fixed to `wrangler deploy --minify --env=""` to remove the ambiguous-environment warning
- `backend/wrangler.jsonc` — extended in Phase 7 with an explicit `"tsconfig": "tsconfig.json"` field, fixing a Wrangler bundler warning that only surfaced in CI (see Phase 7 Edge Cases)
- `backend/tsconfig.json` — trailing comma removed during Phase 7 troubleshooting (turned out not to be the actual root cause, but is objectively invalid JSON and worth having fixed regardless)
- `backend/package.json`, `backend/package-lock.json` — extended in Phase 7 with an explicit `typescript` devDependency (previously silently borrowed from the repo root, which broke in CI's `backend`-only install)

## Next step

Phases 0 (Web OAuth client done; iOS/Android clients still pending, not blocking), 1–7 are done. **Phase 8 is in progress**: NFR-002, the privacy guardrail, and the production promotion gate are all verified. The only remaining work — anywhere in this plan — is the real-device E2E checklist under Phase 8 (EAS builds on physical iOS + Android hardware, token-refresh/background-state check, real Google OAuth click-through). That checklist is entirely human-only; this session has no device, simulator, or EAS-authenticated account to run it with. One loose end from Phase 7 worth a glance on the next real CI run: the `expo/tsconfig.base` and ambiguous-environment Wrangler warnings were fixed but only verified via local `--dry-run` so far, not yet against a real `backend-preview.yml`/`backend-deploy.yml` run.
